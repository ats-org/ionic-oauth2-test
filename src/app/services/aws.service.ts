...
import { Subject, ObjectUnsubscribedError, of, throwError } from 'rxjs';
...
start(){
  console.log(this.configUrl)
    this.brw.create(this.configUrl.baseApi+this.configUrl.loginPath+'?client_id='+this.configUrl.clientId+'&response_type='+this.configUrl.flow+'&redirect_uri=axestrack://axestrack.app/aws', '_system');
 }
   ...
   processAuthCode(code) {
    ...
    return this.getAuthTokenByCode(code, verifier).subscribe(json2 => {
      console.log('getAuthTokenByCode',json2)
        const token = json2;
   ...
   getAuthTokenByCode(code, verifier) {
    ...
    let url = "https://auth.axespointgps.com/oauth2/token";
...
getPixieValues() {
  ...
  const challenge = base64_fixPadding(CryptoES.enc.Base64.stringify(sha));
...
function getRandString() {
  ...
  const randString = base64_fixPadding(btoa(bytes));
  return randString;
}
